...
function loadDetail()
{
    $allDish = $GLOBALS['mysql']->getAll('select * from cake_temp limit 0,1');
    foreach ($allDish as $val) {
        ...
        $GLOBALS['snoopy']->fetch($href . $val['url']);
        exit($GLOBALS['snoopy']->results);
        $html = str_get_html($GLOBALS['snoopy']->results);
        if (empty($html)) {
            echo "no html \n";
            continue;
        }
        $titleEl = $html->find('h1.page-title', 0);
        $data['title'] = '';
        if (!empty($titleEl)) {
            $data['title'] = $titleEl->plaintext;
        }
        $divEl = $html->find('div.recipe-show', 0);
        if (empty($divEl)) {
            echo "no list \n";
            continue;
        }
        $imgEl = $divEl->find('div', 0)->find('img',0);
        $data['img'] = '';
        if (!empty($imgEl)) {
            $data['img'] = $imgEl->src;
        }
        $scoreEl = $divEl->find('div.score',0);
        $data['score'] = '';
        if (!empty($scoreEl)) {
            $data['score'] = $scoreEl->find('span.number',0)->plaintext;
        }
        print_r($data);
    }
}
...
function testC(){
    $allDish = $GLOBALS['mysql']->getAll('select * from category');
    foreach ($allDish as $val) {
        if (empty($val['category_url'])) {
            continue;
        }
        randSnoopy();
        $GLOBALS['snoopy']->fetch($val['category_url'] . 'page/1/');
        $html = str_get_html($GLOBALS['snoopy']->results);
        if (empty($html)) {
            echo "no html \n";
            continue;
        ...
        $divEl = $html->find('div.ui_list_1', 0);
        if (empty($divEl)) {
            echo "no list {$val['category_name']} \n";
            continue;
        }  else{
            echo "good list {$val['category_name']} \n";
            continue;
        }
    }
}
...
function loadTemp()
{
    ...
    foreach ($allDish as $val) {
        ...
        //$href = 'http://home.meishichina.com/recipe/hongbei/page/2/';
        for ($i = 1; $i < 1000; $i++) {
            randSnoopy();
            $GLOBALS['snoopy']->fetch($val['category_url'] . 'page/' . $i .'/');
            $html = str_get_html($GLOBALS['snoopy']->results);
            if (empty($html)) {
                echo "no html \n";
                continue;
            ...
            foreach ($divEl->find('li') as $liEl) {
                $aEl = $liEl->find('h4', 0)->find('a', 0);
                $data['title'] = trim($aEl->plaintext);
                $data['url'] = $aEl->href;
//                print_r($data);
//                print_r($val);
//                exit;
                $dish = $GLOBALS['mysql']->getRow('select * from cake_temp where url = \'' . $data['url'] . '\'');
                if ($dish) {
                    $sql = "UPDATE `cake_temp` SET `name`='" . $data['title'] . "',`url`='" . $data['url'] . "',`cid`='" . $val['category_id'] . "' WHERE `id`='" . $dish['id'] . "'";
                    ...
                    $sql = "INSERT INTO cake_temp (`name`,`url`,`cid`)VALUES('" . $data['title'] . "','" . $data['url'] . "','" . $val['category_id'] . "')";
                    $r = $GLOBALS['mysql']->query($sql);
                    if ($r) {
                        pp($data);
                    }
