...
$allDish = $GLOBALS['mysql']->getAll('select * from gc_58citys');
...
function loadiDesktop()
{
    ...
    $filepath = '/Users/cuihaicheng/Development/Apps/php/ceshi/iDesktop/';
    if (!is_dir($filepath)) {
    ...
    for ($i = 0; $i < 100000000; $i++) {
        $name = $index + $i;
        $url = 'http://s.qdcdn.com/cl/' . $name . ',1280,800.jpg';
        ...
        $fs = filesize($filepath . $filename);
        pp(array($filename, $fs));
        if ($fs == 0) {
